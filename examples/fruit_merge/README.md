...
1. **Body create and destroy.** `fmAddFruit` activates a body slot and sets its
   ...
   one create.
2. **A ContactListener hook.** `fmBeginContact` and `fmEndContact` mirror Box2D's
   `b2ContactListener`. The merge rule lives here. It records same-tier touching
   pairs in insertion order and acts on them after the step, so the body set never
   changes mid-solve.
...
- **RNG.** `fmSplitmix64` is splitmix64 with the reference constants, so the spawn
...
`fm_game.cuh` shows the integration shape against the frozen contract and compiles
against the core. The end-to-end batched launcher (the kernel that drives thousands
of these games and exposes a Python observation API) is assembled once the
narrow-phase, contact solver, and island modules validate. See the repository
README roadmap for what is populated now.
